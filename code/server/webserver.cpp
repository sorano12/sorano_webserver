...
void WebServer::InitEventMode_(int trigMode){
    listenEvent_=EPOLLRDHUP;//检测客户端是否关闭
...
//处理监听套接字，主要逻辑是accept新的套接字，并加入timer和epoller
void WebServer::DealListen_(){
    ...
    do{
        ...
        AddClient_(fd,addr);
    }while(listenEvent_&EPOLLET);//ET模式会一直循环到accept返回-1
}

//处理读事件，主要逻辑就是将onread加入线程池的任务队列
...
void WebServer::OnProcess(HttpConn* client){
    //首先调用process逻辑处理
    if(client->process()){//读完监听写，写完监听读
        epoller_->ModFd(client->GetFd(),connEvent_|EPOLLOUT);//监听写事件
    }
    else{
...
void WebServer::OnWrite_(HttpConn* client){
    ...
    if(client->ToWriteBytes()==0){
        ...
        if(client->IsKeepAlive()){
            epoller_->ModFd(client->GetFd(),connEvent_|EPOLLIN);//监听读事件
            return;
        }
    }
    else if(ret<0){
